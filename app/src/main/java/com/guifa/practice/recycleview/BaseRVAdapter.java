package com.guifa.practice.recycleview;
...
/**
 * Created by wangguifa on 2017/9/12.
 * 基础的adapter
 */
public abstract class BaseRVAdapter<T> extends RecyclerView.Adapter<BaseRVViewHolder> {
    ...
    public BaseRVAdapter(Context context, int itemId, List<T> datas) {
        this.itemId = itemId;
    ...
    public BaseRVViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == TYPE_FOOTER) {
            ...
            return new BaseRVViewHolder(footerView);
        } else if (viewType == TYPE_HEADER) {
            return new BaseRVViewHolder(LayoutInflater.from(mContext).inflate(viewHead, parent, false));
        } else {
            return new BaseRVViewHolder(LayoutInflater.from(mContext).inflate(itemId, parent, false));
        }
    }
    ...
    public abstract void convert(BaseRVViewHolder viewHolder, T item);

    public interface OnItemClickListener {
        void onItemClick(int position, BaseRVViewHolder holder);
